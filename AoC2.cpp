...
int AoC2b(int myArr[16][16]) {
	...
	for (i = 0; i < 16; i++) {
		for (j = 0; j < 16; j++) {
			for (k = j+1; k < 16; k++) {
				if ((myArr[i][j] % myArr[i][k] == 0) || (myArr[i][k] % myArr[i][j] == 0))
					break;
			}

			if (myArr[i][j] % myArr[i][k] == 0) {
				sum += myArr[i][j]/myArr[i][k];
				break;
			}
			else if (myArr[i][k] % myArr[i][j] == 0) {
				sum += myArr[i][k]/myArr[i][j];
				break;
			}
		}
	}
